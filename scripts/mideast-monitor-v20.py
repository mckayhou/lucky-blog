#!/usr/bin/env python3
"""
全球冲突监控脚本 v20 - 量化 + 卫星地图 + GitHub 自动推送
- 新增：GitHub 自动推送（每次报告自动 commit & push）
- 卫星与实时地图模块（LiveUAMap / ISW / Sentinel Hub）
- 时区：北京时间 (UTC+8)
- 置信度：v3 公式 (≥70 推送)
"""
...
SATELLITE_MAPS = {
    'middle_east': [
        ('LiveUAMap 中东实时卫星标注', 'https://liveuamap.com'),
        ('ISW 伊朗/黎巴嫩报告图', 'https://www.understandingwar.org'),
    ],
    'europe': [
        ('LiveUAMap 乌克兰实时地图', 'https://liveuamap.com'),
        ('ISW 俄乌战争每日卫星更新', 'https://www.understandingwar.org/research/russia-ukraine'),
    ],
    'asia': [
        ('LiveUAMap 台海/南海', 'https://liveuamap.com'),
        ('Myanmar Now 缅甸冲突地图', 'https://www.myanmar-now.org'),
    ],
    'americas': [
        ('ACLED 拉美冲突地图', 'https://acleddata.com'),
        ('InSight Crime 墨西哥/中美洲', 'https://insightcrime.org'),
    ],
}
...
def generate_satellite_section():
    """生成卫星地图模块（v19 新增）"""
    section = """### 🛰️ 卫星与实时地图（点击查看高清卫星图像）

"""
    for theater, maps in SATELLITE_MAPS.items():
        theater_name = THEATER_CONFIG[theater]['name'].split('（')[0]
        section += f"**{theater_name}**："
        links = " + ".join([f"[{name}]({url})" for name, url in maps])
        section += f"{links}\n"
...
def generate_report(theater_results, last_run):
    ...
    next_run = beijing_now + timedelta(hours=3)
    ...
    report = f"""## 🌍 全球冲突情报简报（截至 {sg_time} 北京时间）【v19 最终完整版】
...
"""
    ...
    # 卫星地图模块（v19 新增）
    report += generate_satellite_section()
    report += "\n"
...
def push_to_github(report, timestamp):
    """推送报告到 GitHub（v20 新增）"""
    try:
        # 保存报告到 Git 仓库
        report_dir = Path("/root/.openclaw/workspace/reports/global-conflict")
        ...
        # Git 操作
        git_commands = [
            f"cd /root/.openclaw/workspace && git add reports/global-conflict/{report_file.name}",
            f"cd /root/.openclaw/workspace && git -c user.name='Global Conflict Monitor' -c user.email='monitor@localhost' commit -m '📊 全球冲突报告 {timestamp} [auto]'",
        ...
        print(f"✓ 已推送到 GitHub: {report_file.name}")
        return True
    except Exception as e:
...
def main():
    print(f"开始全球冲突监控 v19 (量化 + 卫星地图) - {datetime.utcnow().isoformat()}Z")
    ...
    # 推送到 GitHub（v20 新增）
    push_to_github(report, timestamp)
