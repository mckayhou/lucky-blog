...
"""
    ...
    report += f"""---
**来源交叉验证：** {', '.join(list(sources)[:5]) if sources else 'Tavily 搜索'}
...
def main():
    print(f"开始中东战争监控 v9 (Tavily 版) - {datetime.now().isoformat()}")
    ...
    if not all_results:
        print("未找到任何结果")
        all_results = [{"title": "暂无最新战况", "url": "", "content": "可能网络问题或局势平静"}]
